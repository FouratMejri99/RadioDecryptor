...
import { ChevronDown, ChevronUp, Mic, Radio } from "lucide-react";
...
export function ManualTuning({
  ...
  // ✅ Fetch available frequencies from backend
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch("http://localhost:5000/api/frequencies"); // <- backend port
  ...
  const handleTune = () => {
    ...
    if (freq && freq > 0.1 && freq < 1000) {
      ...
      setShowFrequencyList(true); // Show list after tuning
    }
  };
